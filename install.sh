#!/bin/bash
curl -sS -H "Accept: application/vnd.github.v3+json" -o "/tmp/tmp_file" 'https://get.aoe.workers.dev/repos/qist/xray-ui/releases/latest?host=api.github.com'
...
install_xray-ui() {
    ...
    cd /usr/local/
    if [ $# == 0 ]; then
        wget -N --no-check-certificate -O /usr/local/xray-ui-linux-${arch}.tar.gz https://get.aoe.workers.dev/qist/xray-ui/releases/download/${releases_version}/xray-ui-linux-${arch}.tar.gz?host=github.com
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://get.aoe.workers.dev/qist/xray-ui/releases/download/${releases_version}/xray-ui-linux-${arch}.tar.gz?host=github.com"
    ...
    wget --no-check-certificate -O /usr/bin/xray-ui https://get.aoe.workers.dev/qist/xray-ui/main/xray-ui.sh?host=raw.githubusercontent.com
...
EOF
    ...
    xray-ui restart
    xuilogin() {
        v4=$(curl -s4m8 https://4.ipw.cn/ -k)
        v6=$(curl -s6m8 https://6.ipw.cn/ -k)
        if [[ -z $v4 ]]; then
